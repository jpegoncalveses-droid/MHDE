...
Historical bug log. All issues here are resolved; the live tracker at
`KNOWN_ISSUES.md` (repo root) holds open items only. Each entry preserves
the symptom, root cause, fix, and pointer to the regression test that
prevents recurrence — recoverable context for the future.

Frozen at end of Session 7 (2026-05-07). New bugs go into the live
tracker; when resolved they should be appended here.
...
**Symptom.** When `ml train` succeeded it INSERT'd a new `ml_model_runs`
row but did NOT deactivate prior `is_active=TRUE` rows for the same
`(horizon, target_threshold)` tuple. `crypto/ml/train.py` and
`fx/ml/train.py` already did this; equity didn't. Surfaced concretely
during the Session 7 KI-009 retrain when `ml predict` tried to load
both old (missing) and new joblibs.
**Fix.** Added an UPDATE that deactivates prior actives before the
INSERT in `ml/train.py:train_walk_forward`. Mirrors the existing
pattern in crypto/fx.
**Regression test.** `tests/regression/test_schema_consistency.py::test_active_model_paths_resolve`
walks every `is_active=TRUE` row across all 3 engine `*_model_runs`
tables and asserts each `model_path` exists and `joblib.load`
succeeds. A future train that fails to deactivate stale rows will
surface as missing-joblib errors here.
