...
**Status:** **ACCEPTED 2026-06-14 as ADR-038** (see `DECISIONS.md`) and implemented on
branch `feat/capture-write-then-compact`. Supersedes the ADR-037 in-RAM hourly
roll-up for the firehose writer; ADR-037's retention / inode-guard / migration stand.
This doc is the grounded design basis for that ADR. The firehose stays **parked**
until the §10 measured trial confirms the numbers below.
