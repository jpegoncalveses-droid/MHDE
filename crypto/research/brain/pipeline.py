"""Brain pipeline (Phase 1 step 1): one INERT pass over the capture tape.

Orchestrates the vertical slice — read new aggTrades past the cursor, summarize
the SETTLED windows into raw primitives, persist them to the brain event store,
and advance the cursor + bookkeeping atomically. No continuous runner, no
systemd: :func:`run_once` does exactly one pass and returns.
...
from crypto.research.brain import reader, registry, store, trades

_MS_TO_NS = 1_000_000


def _window_end_ns(trade_time_ms: int, cadence_ns: int) -> int:
...
def run_once(
    *,
    ...
    reader_name: str = cfg.TRADES_READER,
    symbols: Optional[Sequence[str]] = None,
) -> dict:
    """Run one pass; return a summary dict (counts + cursor before/after)."""
    conn = registry.connect(registry_path)
    try:
        cursor_before = registry.get_cursor(conn, reader_name)
        rows = reader.read_new_aggtrades(capture_root, after_recv_ts_ns=cursor_before,
                                         symbols=symbols)

        horizon_ns = now_ns - watermark_ns
        settled, pending = [], []
        for r in rows:
            bucket = settled if _window_end_ns(r["trade_time_ms"], cadence_ns) <= horizon_ns else pending
            bucket.append(r)
        ...
        snapshots = trades.bucket_trades(settled, cadence_ns=cadence_ns)
        seen_by_symbol: dict[str, set] = {}
        fresh = []
        for snap in snapshots:
            sym = snap["symbol"]
            if sym not in seen_by_symbol:
                seen_by_symbol[sym] = registry.seen_windows(conn, cfg.TRADES_DATASET, sym)
            if snap["window_start_ns"] not in seen_by_symbol[sym]:
                fresh.append(snap)

        files = store.write_snapshots(store_root, fresh)
        bookkeeping = [
            {
                "dataset": cfg.TRADES_DATASET,
                "symbol": s["symbol"],
        ...
        registry.advance(conn, reader_name, new_recv_ts_ns=new_cursor,
        ...
        return {
            ...
            "cursor_after": registry.get_cursor(conn, reader_name),
        }
    finally:
        conn.close()
